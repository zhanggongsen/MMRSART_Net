...
def save_images(webpage, visuals, gray_low_dataA, gray_high_dataA, gray_low_dataB, gray_high_dataB, image_path,
                img_spacing,aspect_ratio=1.0, width=128):
    ...
    for label, im_data in visuals.items():
        im = util.tensor2im(im_data, gray_low_dataA, gray_high_dataA, gray_low_dataB, gray_high_dataB, label)
...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        self.saved = False
        # self.img_spacing = [opt.img_spacing,opt.img_spacing,opt.img_spacing]
        self.img_spacing=opt.img_spacing
        if self.display_id > 0:  # connect to a visdom server given <display_port> and <display_server>
            import visdom
            self.ncols = opt.display_ncols
            self.vis = visdom.Visdom(server=opt.display_server, port=opt.display_port, env=opt.display_env)
            # print(self.vis.check_connection())
    ...
    def display_current_results(self, visuals, epoch, save_result):
        if self.display_id > 0:  # show images in the browser using visdom
            ...
            if ncols > 0:  # show all the images in one visdom panel
                ...
                image_numpy_to_x=util.tensor2im(visuals["real_C"], self.opt.gray_low_dataA, self.opt.gray_high_dataA,
                                                     self.opt.gray_low_dataB, self.opt.gray_high_dataB,
                                                     "real_C")
                image_numpy_to_x_1=image_numpy_to_x.transpose([3,0,1,2])
                x = int(np.median(np.array(np.where( image_numpy_to_x_1!= 0))[1]))
                for label, image in visuals.items():
                    image_numpy = util.tensor2im(image, self.opt.gray_low_dataA, self.opt.gray_high_dataA,
                                                 self.opt.gray_low_dataB, self.opt.gray_high_dataB,
                                                 label)
                ...
                images_TobeShow = np.concatenate((images[0],images[1],images[2],images[3]),axis=1)
                ...
                idx = 1
                try:
                    image_numpy_to_x = util.tensor2im(visuals["real_C"], self.opt.gray_low_dataA,
                                                      self.opt.gray_high_dataA,
                                                      self.opt.gray_low_dataB, self.opt.gray_high_dataB,
                                                      "real_C")
                    image_numpy_to_x_1 = image_numpy_to_x.transpose([3, 0, 1, 2])
                    x = int(np.median(np.array(np.where(image_numpy_to_x_1 != 0))[1]))
                    for label, image in visuals.items():
                        image_numpy = util.tensor2im(image, self.opt.gray_low_dataA, self.opt.gray_high_dataA,
                                                     self.opt.gray_low_dataB, self.opt.gray_high_dataB,label)
