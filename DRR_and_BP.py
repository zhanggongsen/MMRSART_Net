...
if True:
    for i in ["0", "50"]:
        path=os.path.join(root,"Cropped_{}".format(i))
        # Use meter as unit in geometry definition
        phantom_origin = sitk.ReadImage(os.path.join(path, "image_removed_couch.nii"))
        ...
        Body = sitk.ReadImage(os.path.join(path, "mask_Body.nii"))
        ...
        sitk.WriteImage(img_drr_without_noise, os.path.join(path, 'DRR_no_noise.nii'))
        ...
        img_drr_noise.SetSpacing([(detector_side[0] * 1000 / detector_size[0]) * sad / sid,
                                  (detector_side[1] * 1000 / detector_size[1]) * sad / sid, 1])
        sitk.WriteImage(img_drr_noise, os.path.join(path, 'DRR_noise.nii'))
        ...
        sitk.WriteImage(img_3DDRR, os.path.join(path, "3DDRR_origin.nii"))
        ...
        sitk.WriteImage(img_Recons_without_noise, os.path.join(path, "recons_origin_no_noise.nii"))
        ...
        sitk.WriteImage(img_Recons_noise, os.path.join(path, "recons_origin_noise.nii"))
